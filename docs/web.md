...
│   │   ├── page.tsx            トップ
│   │   ├── audit/              監査ログ閲覧画面 (cursor pagination)
...
`/audit` は `audit_logs` テーブルを `id DESC` で読み出す SSR ページ。cursor pagination (`?cursor=<id>`) で過去方向にページング。記録は `memberActionClient` / `authActionClient` の middleware (`safe-action.ts`) と bot 側 outbox 送信フックから `writeAuditLog()` 経由で `audit_logs` に INSERT される。
